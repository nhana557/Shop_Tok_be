...
const productController = {
	...
	insertProduct: async (req, res) => {
		...
		insert(data)
			.then((result) => response(res, data, 201, 'Product created'))
			.catch((err) => {
				console.log(err);
				res.send(err);
			});
	...
	updateProduct: async (req, res, next) => {
		try {
			...
			update(data)
				.then((result) => response(res, data, 200, 'Product updated'))
				.catch((err) => res.send(err));
		} catch (error) {
			console.log(error);
			next(error);
	...
	deleteProduct: async (req, res, next) => {
		try {
			...
			deleteData(id)
				.then((result) => response(res, result.rows, 200, 'Product deleted'))
				.catch((err) => res.send(err));
		} catch (error) {
			console.log(error);
			next(error);
